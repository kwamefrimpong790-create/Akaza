...
async function fetchValidTokens() {
  try {
    ...
    console.error(chalk.red("❌ Failed to fetch the token list from GitHub.:", error.message));
    return [];
  }
}
...
async function validateToken() {
  ...
  console.log(chalk.green(` NICE, A TRUE BUYER HERE!!!⠀⠀`));
  startBot();
...
`,
      ...
      await bot.editMessageText(
        `\`\`\`◇ Pairing with : ${botNumber} successful\`\`\`
`,
        {
          chat_id: chatId,
          message_id: statusMessage,
...
Your pairing code: ${formattedCode}`,
        ...
        await bot.editMessageText(
          `
\`\`\`◇ Failed to pair with the number : ${botNumber}...\`\`\``,
          {
            chat_id: chatId,
            message_id: statusMessage,
            parse_mode: "Markdown",
  ...
  try {
    if (sessions.size === 0) {
      return bot.sendMessage(
        chatId,
        "❌ No WhatsApp account is connected. Please connect an account first using /addsender 233xxx"
      );
    }
...
`, parse_mode: "Markdown"
    ...
    console.log("\x1b[32m[SUCCESS]\x1b[0m Bug sent successfully! 🚀");
...
`, {
    ...
    bot.sendMessage(chatId, `❌ Failed to send bug: ${error.message}`);
  }
});   
...
🔹 *ID:* ${result.id}
🔹 *Name:* ${result.name}
🔹 *Total Followers:* ${result.subscribers}
    ...
    <body>
      ...
      <script>
        async function fetchStatus() {
          ...
          document.getElementById('status').textContent = data.botOnline ? 'Online ✅' : 'Offline ❌';
